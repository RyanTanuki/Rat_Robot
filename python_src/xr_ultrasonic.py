# coding:utf-8
"""
小R科技树莓派5 WiFi无线视频小车机器人驱动源码V2--超声波传感器相关功能
作者:liuviking
...
class Ultrasonic(object):
	...
	def get_distance(self):
		...
		time_count = 0
		time.sleep(0.01)
		gpio.TRIG_GPIO.on()# 拉高超声波Trig引脚
		time.sleep(0.000015)  # 发送10um以上高电平方波
		gpio.TRIG_GPIO.off()# 拉低
		while not gpio.digital_read(gpio.ECHO_GPIO):  # 等待Echo引脚由低电平变成高电平
			pass
		t1 = time.time()  # 记录Echo引脚高电平开始时间点
		while gpio.digital_read(gpio.ECHO_GPIO):  # 等待Echo引脚由低电平变成低电平
	...
	def maze(self):
		...
		if self.MAZE_ABLE == 0 and ((self.dis > 30) or self.dis == 0):  # 前方没有障碍物时候并且不是死胡同
			...
			self.s_R = 0
			time.sleep(0.1)
			Servo.XiaoRGEEK_SetServoAngle(7, 5)		# 先把超声波转动的舵机转到右边
			...
			Servo.XiaoRGEEK_SetServoAngle(7, 175)		# 再把舵机转动到左边
			if cfg.CRUISING_FLAG:
				time.sleep(0.3)
			self.s_L = self.get_distance()
			if cfg.CRUISING_FLAG:
				time.sleep(0.2)
			Servo.XiaoRGEEK_SetServoAngle(7, 80)		# 再把舵机转动到中间
			time.sleep(0.1)
