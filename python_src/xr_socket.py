# coding:utf-8
"""
小R科技树莓派5 WiFi无线视频小车机器人驱动源码V2---TCP命令解析,手机遥控命令接收
作者:liuviking
	...
	def communication_decode(self, buffer):
		...
		if buffer[0] == 0x00:  # buffer[0]表示类型位，等于0x00表示这个数据包为电机控制指令的数据包
			...
			if abs(cfg.SERVO_ANGLE - cfg.SERVO_ANGLE_LAST) > 2:  # 限制舵机重复下发角度
				...
				Servo.XiaoRGEEK_SetServoAngle(cfg.SERVO_NUM, cfg.SERVO_ANGLE)
			...
			if buffer[1] == 0x01:
				...
				cfg.COLOR_INDEX = cfg.COLOR_FOLLOW_SET['none']
				print("This color is not support in DS V2!")
			time.sleep(1)
			...
			if buffer[1] == 0x01 and cfg.CRUISING_FLAG == cfg.CRUISING_SET['normal']:
				...
				cfg.LASRT_LEFT_SPEED = cfg.LEFT_SPEED  # 将当前速度保存
				cfg.LASRT_RIGHT_SPEED = cfg.RIGHT_SPEED
				Servo.XiaoRGEEK_SetServoAngle(7, 90)
				Servo.XiaoRGEEK_SetServoAngle(8, 0)
			...
			elif buffer[1] == 0x00 or buffer[1] == 0x07:		# 正常模式
